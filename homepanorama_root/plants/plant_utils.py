...
from .serializers import (
    ...
    PlantDataSerializer,
    PlantSerializer,
    SoilFertitlityBordersSerializer,
    SoilMoistureBordersSerializer,
    SunlightIntensityBordersSerializer,
    TemperatureBordersSerializer,
    ValueChangesSerializer,
)
...
def get_plant_details(plantdata, plant_id):
    ...
    soil_fertitlity_borders_serializer = SoilFertitlityBordersSerializer(
        soil_fertitlity_borders
    )
    soil_moisture_borders = SoilMoistureBordersSerializer(soil_moisture_borders)
    sunlight_intensity_borders = SunlightIntensityBordersSerializer(
        sunlight_intensity_borders
    )
...
def get_plant_history(plant_id, hours):
    ...
    plant_list = (
        PlantData.objects.filter(plant_id=plant_id)
        .filter(timestamp__gt=minimum_datetime)
        .order_by("timestamp")
    )
    plants_dict = {}
    counter = 0
    if plant_list.count() > 0:
        for plant in plant_list:
            plantdata = PlantData.objects.filter(
                plant_id=plant_id, timestamp=plant.timestamp
            ).first()
    ...
    results_m = ValueChanges.objects.filter(
        plant_id=plant_id, value="M", date__gte=start_seven_days_ago
    )
    results_m = [value_changes.date.date() for value_changes in results_m]
    results_f = ValueChanges.objects.filter(
        plant_id=plant_id, value="F", date__gte=start_seven_weeks_ago
    )
    results_f = [value_changes.date.date() for value_changes in results_f]

    plants_dict["pastWaterReviewArray"] = get_hits(
        results_m, 7, start_seven_days_ago, 1
    )
    plants_dict["pastFertilizerReviewArray"] = get_hits(
        results_f, 7, start_seven_weeks_ago, 7
    )
...
def get_hits(result_list, range_in_days, start_datetime, interval):
    ...
    for x in range(range_in_days):
        ...
        elif interval == 7 and any(
            date in result_list
            for date in [
                start_datetime.date() + timedelta(days=x) for x in range(interval)
            ]
        ):
            past_reviews.append(1)
        else:
...
def get_all_plant_details():
    ...
    for plant_id_dic in plant_list_id:
        try:
            ...
            plant = Plant.objects.filter(id=plant_id).first()
            soil_fertitlity_borders = SoilFertitlityBorders.objects.get(
                plant_id=plant.id
            )
            soil_moisture_borders = SoilMoistureBorders.objects.get(plant_id=plant.id)
            sunlight_intensity_borders = SunlightIntensityBorders.objects.get(
                plant_id=plant.id
            )
            temperature_borders = TemperatureBorders.objects.get(plant_id=plant.id)
            ...
            plant_dict[
                "soil_fertitlity_borders"
            ] = soil_fertitlity_borders_serializer.data
            plant_dict["soil_moisture_borders"] = soil_moisture_borders.data
            plant_dict["sunlight_intensity_borders"] = sunlight_intensity_borders.data
            plant_dict["temperature_borders"] = temperature_borders.data
...
def data_cleanup(plant_id):
    ...
    old_plant_data = PlantData.objects.filter(
        plant_id=plant_id, timestamp__lt=fourteen_days_ago
    )
    if old_plant_data:
...
def checkup_plant_data(new_plant_data, old_plant_data):
    print("--------------")
    print(
        str(new_plant_data["soil_moisture"])
        + " == ?"
        + str(old_plant_data["soil_moisture"])
    )

    if data_checkup(
        new_plant_data["soil_moisture"], old_plant_data["soil_moisture"], 2
    ):
        new_entry_date = dateutil.parser.parse(new_plant_data["timestamp"])
        if not ValueChanges.objects.filter(
            date__date=new_entry_date.date(),
            value="M",
            plant_id=new_plant_data["plant_id"],
        ).first():
            related_plant = Plant.objects.filter(id=new_plant_data["plant_id"]).first()
            ValueChanges.objects.create(
                value="M", date=new_plant_data["timestamp"], plant=related_plant
            )

    if data_checkup(
        new_plant_data["soil_fertility"], old_plant_data["soil_fertility"], 2
    ):
        new_entry_date = dateutil.parser.parse(new_plant_data["timestamp"])
        if not ValueChanges.objects.filter(
            date__date=new_entry_date.date(), value="F"
        ).first():
            related_plant = Plant.objects.filter(id=new_plant_data["plant_id"]).first()
            ValueChanges.objects.create(
                value="F", date=new_plant_data["timestamp"], plant=related_plant
            )
