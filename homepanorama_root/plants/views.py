    ...
    if request.method == "GET":
        ...
        if PlantData.objects.filter(plant_id=plant_json["plant_id"]).first():
            plantdata = PlantData.objects.filter(
                plant_id=plant_json["plant_id"]
            ).latest("timestamp")
            if check_for_reset(plant_json["plant_id"]) is False:
                last_sunlight_data = plantdata.sunlight
            last_documented_plant_data = get_plant_details(
                plantdata, plant_json["plant_id"]
            )
        ...
        PlantData.objects.create(
            battery=plant_json["battery"],
            soil_fertility=plant_json["soil_fertility"],
            soil_moisture=plant_json["soil_moisture"],
            sunlight=last_sunlight_data + int(plant_json["sunlight"]),
            temperature=plant_json["temperature"],
            timestamp=plant_json["timestamp"],
            plant=plant,
        )
        # except:
...
def reload_plant_data(request):
    ...
    child = pexpect.spawn(
        "ssh hoobs@192.168.1.79 sudo python3 /AutomationTools/Tools/miflora_reader.py"
    )
...
def create_update_plant(request, plant_id):
    if request.method == "PUT":
        ...
        if Plant.objects.filter(id=plant_id).first() is not None:
            ...
            Plant.objects.filter(id=plant_id).update(
                name=plant_json["name"], address=plant_json["address"]
            )
        else:
            print("create exisiting plant")
            Plant.objects.create(
                id=plant_id, name=plant_json["name"], address=plant_json["address"]
            )

        SoilFertitlityBorders.objects.filter(plant_id=plant_id).update_or_create(
            min=plant_json["soil_fertitlity_borders"]["min"],
            max=plant_json["soil_fertitlity_borders"]["max"],
        ...
        SoilMoistureBorders.objects.filter(plant_id=plant_id).update_or_create(
            ...
            currency=plant_json["soil_moisture_borders"]["currency"],
        )
        SunlightIntensityBorders.objects.filter(plant_id=plant_id).update_or_create(
            ...
            currency=plant_json["sunlight_intensity_borders"]["currency"],
        )
        TemperatureBorders.objects.filter(plant_id=plant_id).update_or_create(
            ...
            currency=plant_json["temperature_borders"]["currency"],
        )
        Locations.objects.filter(plant_id=plant_id).update_or_create(
            location=plant_json["location"]["location"],
            location_details=plant_json["temperature_borders"]["max"],
        )
